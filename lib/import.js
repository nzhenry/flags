	...
	return author
		? storePlace(author.place)
				.then(place => {
					...
					var values = {place: place.id};
					if(typeof author == 'string') values.name = author;
					else if(author.name) values.name = author.name;
					return db.upsert('authors', ['name','place'], values);
				}).then(x=>x.rows[0])
		: Promise.resolve();
}
